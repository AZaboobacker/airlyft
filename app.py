...
if submitted:
    ...
    with st.spinner("Generating code..."):
        ...
        message_content = response.choices[0].message.content.strip() - Do not use "ChatCompletion.create" because it gives error so Use "chat.completions.create()" only instead of "ChatCompletion.create" and message_content = response.choices[0].message.content.strip() instead of message_content = response.choices[0].message['content'].strip()"""}
                ]
            )
